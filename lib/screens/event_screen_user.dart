...
class EventScreenUser extends StatefulWidget {
  final String role;
  final String idServer;

  EventScreenUser({required this.role, required this.idServer});
  @override
  _EventScreenUserState createState() => _EventScreenUserState();
}

class _EventScreenUserState extends State<EventScreenUser> {
  final log = Logger('EventScreenUser');
  ...
  void initState() {
    super.initState();
    _eventSessions = Future.value([]);
    _refreshEvent();
  }

  void _refreshEvent() async {
    log.info('Refreshing event sessions');
    ...
    if (jsonClientIds != null) {
      ...
      // Dekode JSON
      clientIds = List<String>.from(json.decode(jsonClientIds));
      log.info('Client IDs retrieved: $clientIds'); // Log ID klien yang diambil
    } else {
      log.warning(
          'No client IDs found in login table.'); // Log jika tidak ada ID klien
    }

    // Ambil event sessions berdasarkan client IDs
    if (clientIds.isNotEmpty) {
      _eventSessions =
          DatabaseHelper.instance.getEventSessionsForClients(clientIds);
      ...
      log.warning(
          'No client IDs to fetch events for.'); // Log jika tidak ada ID klien
      // Jika tidak ada client ID, bisa mengosongkan data atau menampilkan pesan
      setState(() {
        _allData = [];
        _filteredData = [];
      });
  ...
  Future<List<Map<String, dynamic>>> _fetchClients() async {
    ...
    List<Map<String, dynamic>> _clients = []; // Inisialisasi daftar klien
    ...
    if (jsonClientIds != null) {
      ...
      for (String clientId in clientIds) {
        log.info('Fetching client with ID: $clientId'); // Log pengambilan klien
    ...
    return _clients; // Kembalikan daftar klien
  }
